...
for (let radar of liste_radars){
    ...
    radar.addEventListener("dragstart", prendre_radar)
    radar.addEventListener("touchstart", prendre_radar)
}
...
//         if (radar_drag && radar_drag.div_distance_vecteur != null){
//             div_video.removeChild(radar_drag.div_distance_vecteur)
//             radar_drag.div_distance_vecteur = null
//         }
//     })
// }


function prendre_radar(event){
    radar_drag = event.target
    dessiner_autres_vecteurs(radar_drag)
    if (radar_drag.div_distance_vecteur != null){
        div_video.removeChild(radar_drag.div_distance_vecteur)
        radar_drag.div_distance_vecteur = null
    }
    setTimeout(function(){
        div_video.addEventListener("touchstart", lacher_radar_video)}, 300)
    
}

function lacher_radar_dropzone(){
    if (radar_drag!=null){
...
div_radars.addEventListener("drop", lacher_radar_dropzone)

function lacher_radar_video(event){
    let rect = canvas_video.getBoundingClientRect()
    let x = event.clientX - rect.left + scrollX
    let y = event.clientY - rect.top + scrollY
    if (isNaN(x) && isNaN(y)){
        x = event.targetTouches[0].clientX - rect.left + scrollX
        y = event.targetTouches[0].clientY - rect.top + scrollY
    }
...
function placer_vecteur(event){
    ...
    checkbox_distance_vecteur.addEventListener("change", function(){
        if (checkbox_distance_vecteur.checked && !isNaN(parseInt(input_distance_vecteur.value))){
...
function dessiner_rectangle_max(){
...
function dessiner_rectangle_min(){
...
div_dessiner_rectangle_max.addEventListener("click", dessiner_rectangle_max)

div_dessiner_rectangle_min.addEventListener("click", dessiner_rectangle_min)
