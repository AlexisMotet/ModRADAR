...
function tracer_FFT(resultats_dsp_FFT, fft_selec, mode_affichage, callback){
    ...
    for (let [nom_fft, fft] of Object.entries(resultats_dsp_FFT)){
        if ((fft_selec == "triangle1" && (nom_fft == "fft_m1" || nom_fft == "fft_d1"))
            ||(fft_selec == "triangle2" && (nom_fft == "fft_m2" || nom_fft == "fft_d2"))){
            if (nom_fft == "fft_m1" || nom_fft == "fft_m2"){
                color = "orange"
                color_CA_CFAR = "blue"
                color_pics = "darkorange"
            }
            else if (nom_fft == "fft_d1" || nom_fft == "fft_d2"){
                color = "green"
                color_CA_CFAR = "red"
...
function tracer_droites(resultats_dsp_associations, callback){
    ...
    if (x.length == 0 && y.length == 0){
        plot_points_potentiels = {
            ...
            marker: {size : 15, symbol : "square-open", color : "yellowgreen", line : {
                width : 3}}
    ...
    if (x.length == 0 && y.length == 0){
        plot_points_certains = {
            ...
            marker: {size : 15, symbol : "square-open", color : "orange", line : {
                width : 3}}
        }
    }
    else{
